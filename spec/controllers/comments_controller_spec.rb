...
RSpec.describe CommentsController, type: :controller do
  ...
  context "member user doing CRUD on a comment they don't own" do
    ...
    describe "POST create" do
      it "increases the number of comments by 1" do
        expect{post :create, format: :js, post_id: my_post.id, comment: {
        body:RandomData.random_paragraph}}.to change(Comment,:count).by(1)
      end
  ...
  context "memeber user doing CRUD on a post they own" do
     before do
    ...
    describe "DELETE destory" do
      it "delete the comment" do
        delete :destroy, format: :js, post_id: my_post.id, id: my_comment.id
        count = Comment.where({id: my_post.id}).size
